...
Way back in 2012 I started working on a project called [Privatesquare](https://straup.github.io/privatesquare/). At the
same I was doing a lot of work around the idea of [personal archiving](https://www.aaronland.info/weblog/2012/02/14/incentivize/#pda2012) and
...
morphed in to a tool where I would _first_ check-in to Privatesquare and then
...
[New York Public Library](https://github.com/NYPL/gazetteer/wiki/ApiDocs) or so-called "states of mind"). It also added the ability to record travel
...
services to get thing done: [Foursquare](https://developer.foursquare.com/) for venues and search; [Stamen](http://maps.stamen.com/) for map
tiles; The [Flickr API](https://www.flickr.com/services/api) for geocoding cities.

The first week I started at Mapzen, in 2015, I remembering thinking: I wonder if I can
...
Everything in Privatesquare is private to an individual. There was (is) no way
...
a few people did) if you used my instance everyone's data was stored together in
a shared database that someone else (me) controlled. So it was "private" in as
much as you trust me.

That's a truism bordering on a platitude but in 2018 [trust in systems, if not
individuals, feels like it is sometimes in short supply](https://www.aaronland.info/weblog/2012/10/08/signs/#stories). Every day seems to
bring news of some previously unknown existential flaw in the fundamental
principles of computers and computer networks – aka "the cloud" – so that even if you
...
problem of course is that people now have multiple computers – aka "devices" –
and there is an expectation that they can synchronize data between one another.
...
company of Firefox, doesn't know what's in your bookmarks.

It's not a perfect solution but it feels like forward motion and better than yesterday.

---

It's also worth at least mentioning [differential
...
they can resell, whether its advertising or genuinely useful secondary services.

What I like about projects like differential privacy and federated learning is
...
benefits and trade-offs in all of this.
...
install software to manipulate those tools are readily available in 2018.
...
It is useful check and a separation of concerns that forces an application to
succeed on its merits and not because it has the key all your data.

---

One day, after showing some work-in-progress of my new Mapzen-enabled
Privatesquare – aka "Privatezen" – during a staff all-hands a colleague asked if
...
It is a thing I've always missed since I stopped running a hosted version of
Privatesquare and I've always felt that there was no point in doing Who's On
First unless you could actually _do_ something with the data so this was a good
way to kick the tires.

Briefly, the application is organized around six categories: Visits, Neighbourhoods,
Cities, Lists, #feelings and Trips.
...
All of these categories hold hands with each other meaning you
...
Chief among the long list of short-cuts and gotchas is that search has never worked very well, for three
reasons.

First lots of venues (in Who's On First) hadn't been indexed in Mapzen
Search. Second, we were using the [mapzen.js](https://github.com/nextzen/mapzen.js) library for map display and the
...
[Pelias-API compliant API](https://github.com/whosonfirst/whosonfirst-www-api/blob/master/www/include/lib_api_whosonfirst_pelias.php) it was always buggy and those fixes just never made it
...
then, using the special syntax `id:{WHOSONFIRST_ID}`,  pasting it to the search
...
There are couple things to note here: There was nothing venue specific about
...
you can use the application to signal places that haven't even been to yet.

![](images/transit.png)

I mentioned that I was trying to integrate as many of the various Mapzen services, not
...
[Transitland API](https://transit.land/) for nearby transit stops.

Like a lot of features this never moved beyond the basic prove-it-works-with-a-list stage on to a more refined and
...
thumb was to add "a handy map" to almost every view, whenever possible.
...
City](https://places.whosonfirst.org/id/ 85977539/). As you can
see from the many dots on the map and only three visible "visits" another of the
...
Careful readers may also notice that this is an older screenshot from a time when the
search box was in the bottom right, instead of the top, and "#feelings" were
still called "desires".
...
It was also possible to create lists and like the transit work, above, these
...
go there" #feeling. This is a straight clone of [the trips functionality in
...
by a completely different map "layer" managed by [Leaflet](http://leafletjs.com/) and the `tangram.js` code
doesn't even know that those features exist to draw them.

![](images/browse.png)

I mentioned earlier that there's never been a proper interface for list
views. Browsing nearby all the venues surrounding a point is a good example of that.

![](images/yesnofix.png)

Here's a screenshot of some work that I started to to do to a generic view (or
what are sometimes referred to as "cards") for any given place. This included
developing a range of colour-codings to apply to things like dots on the help,
to help indicate whether a venue was open or closed.

We also thought that these "browse" views could do double-duty as a place where
...
and `superseded_by` properties in action. These two restaurants have occupy the same
...
tricks that could be applied from John's work. If nothing else John is drawing all
...
even if they were make for _really boring_ screenshots. Most importantly the goal
is to work offline.
...
much everything except search and maps (discussed below) work as expected.
...
is that the screenshot will be loaded _underneath_ the map. If you're online or
...
search doesn't work but even you happen to know the Who's On First ID of the
place you're trying to add the application needs to fetch the details for that
place over the wire. Now that we've started building [bundled SQLite databases
of Who's On First data](/download/#sqlite) it's easy to imagine a scenario where
you can download one or more "bundles" specific to your interests, say just the
venues in Berlin, and those are used for lookups if the API isn't available.

---

The other place where it doesn't "work" is adding new venues that aren't already
in Who's On First. It would be really useful for people to have a mechanism that
...
for now. There are the beginnings of [a
Cordova-based
...
would have invented something like React too. I didn't though so it's just a
whole lot of JavaScript libraries that load and call one another and I would be
willing to accept that I'm "doing it wrong".

Part of building this tool became an exercise in trying to refactor or write all
...
a [nodejs](https://nodejs.org/) context. They don't yet, or it some of them do then not all of them
do and probably not well, but that is the goal.

I've been loosely calling the project "Privatezen" in this blog but the actual
code is stored in a repository with the admittedly dull name of [electron-whosonfirst-bookmarks](https://github.com/whosonfirst/electron-whosonfirst-bookmarks).
...
The gem that fell out of that meeting was the idea for a brain-dead simple mobile
...
Ideally we would have wanted for people to go back and fill the names and
...
scribbled out on the whiteboard that afternoon wasn't erased for 18 months
...
and the usefulness in the silliness, of those framing devices. 
...
It's something I've tried to remember while building "Privatezen".
